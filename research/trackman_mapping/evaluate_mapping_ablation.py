...
def legacy_crosswalk(tm, tr):
    ...
    return (
        v.sort_values(["pitcher_trackman_id", "votes", "pitcher_id"])
...
def trackman_wide(tm, xw):
    ...
    z["wx"] = z["score"] * z["w"]
    wide = (
        z.groupby(["pitcher_id", "season"])
        .agg(
            spin_stability_score=("wx", "sum"),
            tm_n=("n", "sum"),
...
def attach(df, wide, suffix):
    ...
    w = wide.copy()
    w["season"] = w["season"].astype(float)
    m = pd.merge_asof(
        q.sort_values("query_season"),
        w.sort_values("season"),
    ...
    grade = np.where(m["tm_n"].isna(), "D", np.where(m["tm_purity"] >= 0.9, "A", "B"))
...
def main():
    ...
    tr = pd.read_csv(
        DATA / "train.csv",
        usecols=list(dict.fromkeys(features + KEY + ["pitcher_id", "control_success"])),
    )
    tm_cols = list(
        dict.fromkeys(
            KEY
    ...
    print(
        f"legacy pitchers={len(old)}, purity={old.purity.mean():.6f}; "
        f"deterministic={len(new)}, purity={new.purity.mean():.6f}"
    )
    ...
    for c in cats:
        base[c] = base[c].fillna(-1).astype(str)
    base[[c for c in features if c not in cats]] = (
        base[[c for c in features if c not in cats]].fillna(0.49).astype("float32")
    )
    ...
    for name, extra in [("legacy", fo), ("deterministic", fn)]:
        X = base.copy()
        X["spin_stability_score"] = extra.iloc[:, 0].to_numpy()
        X["data_confidence_grade"] = extra.iloc[:, 1].to_numpy()
        ...
        row = {"mapping": name, **metrics(y[val], p)}
        results.append(row)
        print(row)
